...
class AuditNations extends Job implements ShouldQueue
{
    ...
    protected function determineMMRMessage()
    {
        ...
        {
            $this->message .= "-- You haven't signed in yet. Please go [link=http://www.rnco.uk/signin]here[/link] to sign in \n";
        }
        else
        {
            // If they're garbage
            ...
            {
                $this->needsMMRAudit();
                $this->message .= "-- You [b]REALLY[/b] do not meet your warchest requirements. This needs to be your #1 priority. Go [link=https://www.rnco.uk/mmr]here[/link] to check your requirements. Your MMR Score is {$this->mmr->mmrScore} \n";
            }
            elseif ($this->mmr->mmrScore < 80 && $this->nation->cities > 7) // If they're not so bad
            {
                $this->needsMMRAudit();
                $this->message .= "-- You are on your way to meeting your warchest requirements, however you aren't close enough. Please work on meeting your warchest requirements ASAP. Go [link=https://www.rnco.uk/mmr]here[/link] to check your requirements. Your MMR Score is {$this->mmr->mmrScore} \n";
            }
            elseif ($this->mmr->mmrScore < 100 && $this->nation->cities > 7) // If they're pretty good
            {
                $this->needsMMRAudit();
                $this->message .= "-- You are almost meeting your warchest requirements, but you aren't quite there. Please work on meeting your warchest requirements ASAP. Go [link=https://www.rnco.uk/mmr]here[/link] to check your requirements. Your MMR Score is {$this->mmr->mmrScore} \n";
            }
