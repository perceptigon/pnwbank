...
class mlpGrants extends Model
{
    use DispatchesJobs;

    protected $table = "mlpgrants";

    /**
     * Return all pending mlp Grants.
     *
     * @return Collection
     */
    ...
    public static function acceptGrant(int $gID)
    {
        ...
        $message = "Hi $grant->leader, \n \n Your EGR grant has been sent to you. Thank you!";

        dispatch(new SendMoney($bank, $grant->leader, "Your EGR Grant has been Approved!", $message));
        ...
        $profile->gottenmlpGrant = true;
        $profile->save();

        Log::createLog("mlp", "Approved - $grant->id");
    }
    ...
    public static function denyGrant(int $gID)
    {
        ...
        $grant->isPending = false;
        $grant->save();
        Log::createLog("mlp", "Denied - $grant->id");
        // TODO send them a message
    }

    /**
     * Checks if a nation ID has a pending mlp Grant.
     *
     * @param int $nID
     * @return bool
     */
